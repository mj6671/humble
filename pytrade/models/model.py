...
def update_order_status(user_id, order_id, status, exchange_order_status):
    conn = db_connect()
    cursor = conn.cursor()
    cursor.execute("UPDATE trades SET status=%s,exchange_order_status=%s WHERE id=%s AND user_id=%s", (status, exchange_order_status, order_id, user_id))
    conn.commit()
    conn.close()
# Fetch user API keys
...
def log_trade(user_id, order_id, exchange_id, exchange_order_id, symbol, order_type, type, price, amount, status):
    conn = db_connect()
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO order_book (user_id, order_id, exchange_id, exchange_order_id, currency, order_type, type, price, qty, status) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
        (user_id, order_id, exchange_id, exchange_order_id, symbol, order_type, type, price, amount, status)
    )
    conn.commit()
    conn.close()
...
def get_open_order(order_id):
    ...
    try:
        with db.cursor(dictionary=True) as cursor:
            cursor.execute("SELECT status FROM trades WHERE id = %s", (order_id,))  # Ensure tuple
