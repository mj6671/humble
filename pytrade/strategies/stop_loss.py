def calculate_variable_percentage_stop_loss(order_price, percentage, order_type):
    """
    Calculates stop-loss based on a percentage of the executed order price.
    """
    if order_type == "BUY":
        return order_price * (1 - percentage / 100)  # Stop-loss is below for BUY orders
    elif order_type == "SELL":
        return order_price * (1 + percentage / 100)  # Stop-loss is above for SELL orders


# Last Candle Stop-Loss

def calculate_last_bearish_candle_stop_loss(candles):
    """
    Calculates stop-loss for BUY orders based on the last bearish candle's low.
    """
...
def calculate_rsi_based_stop_loss_for_buy(candles, rsi_values, rsi_lower_limit):
    """
    ...
    for i in reversed(range(len(candles))):
        if rsi_values[i] < rsi_lower_limit and candles[i]["close"] < candles[i]["open"]:  # RSI below lower limit and bearish
            return candles[i]["low"]


def calculate_rsi_based_stop_loss_for_sell(candles, rsi_values, rsi_upper_limit):
    """
    Calculates stop-loss for SELL orders based on the last bullish candle with RSI above the upper limit.
    """
    for i in reversed(range(len(candles))):
        if rsi_values[i] > rsi_upper_limit and candles[i]["close"] > candles[i]["open"]:  # RSI above upper limit and bullish
            return candles[i]["high"]


# Last N Candles Stop-Loss
...
def calculate_lowest_low_for_last_n_candles(candles, n):
    ...
    return min(candle["low"] for candle in last_n_candles)
...
def calculate_highest_high_for_last_n_candles(candles, n):
    ...
    """
    last_n_candles = candles[-n:]
    return max(candle["high"] for candle in last_n_candles)


# Calculate Stop-Loss

def calculate_combined_stop_loss(order_price, percentage, candles, rsi_values, rsi_lower_limit, rsi_upper_limit, order_type):
    """
    ...
    if order_type == "BUY":
        ...
        rsi_stop_loss = calculate_rsi_based_stop_loss_for_buy(candles, rsi_values, rsi_lower_limit)
        ...
        rsi_stop_loss = calculate_rsi_based_stop_loss_for_sell(candles, rsi_values, rsi_upper_limit)
