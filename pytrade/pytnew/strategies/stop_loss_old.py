...
def calculate_rsi_based_stop_loss_for_buy(candles, rsi_lower_limit):
    ...
    for i in reversed(range(len(candles))):
        if candles[i]["rsi"] < rsi_lower_limit and candles[i]["close"] < candles[i]["open"]:  # RSI below lower limit and bearish
...
def calculate_rsi_based_stop_loss_for_sell(candles, rsi_upper_limit):
    ...
    """
    for i in reversed(range(len(candles))):
        if candles[i]["rsi"] > rsi_upper_limit and candles[i]["close"] > candles[i]["open"]:  # RSI above upper limit and bullish
